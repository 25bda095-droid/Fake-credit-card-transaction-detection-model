...
with main_col:
    ...
    if st.session_state.show_results and st.session_state.analysis_results:
        ...
        if y_true is not None:
            ...
            for name, color in zip(models, colors):
                ax3.plot(np.array(res["fprs"][name]), np.array(res["tprs"][name]), color=color, lw=2, label=f"{name} (AUC = {aucs[name]:.4f})")
            ...
            bars = ax4.bar(models, recall_vals, color=colors, edgecolor='black', linewidth=1.5)
        ...
        comp_data = {
            ...
            'ROC AUC': [f"{aucs[n]:.4f}" for n in models] if y_true is not None else [""]*len(models)
        }
